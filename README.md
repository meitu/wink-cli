...
设备编号 `gnum` 必须是发号服务分配的正整数（int64）。CLI 优先使用 `WINK_TASK_GNUM`，其次读取本地 `task-gnum`；旧缓存为十六进制时，会尝试迁移旧 SDK `datareport/dataReport.json` 中的有效 `gid`。没有已分配编号时会明确报错，不生成随机编号；新设备发号接口仍待接入。
