...
(async () => {
  ...
  try {
    ...
    const extras = {
      ...
      ai_translation: ["--target-language", "en"], ai_beauty: ["--style", "67201"], video_frame: ["--fps", "60"],
    };
    let count = 0;
    for (const row of rows) {
      ...
      assert.strictEqual(JSON.parse(submitted.right_detail).function_id, command === "ai_beauty" ? "672" : "999");
      assert.ok(uploads.every(x => x.test === true));
      assert.deepStrictEqual(calls, [...(command === "ai_beauty" ? ["/material/ai_beauty/list"] : []), "/task/ai_type_config", "/task/submit", "/task/query"]);
      ...
      if (command === "ai_beauty") {
        assert.deepStrictEqual(JSON.parse(params.retouch_ai_params), { beauty_style: { smoothing: 0.5 } });
        assert.deepStrictEqual(JSON.parse(submitted.right_detail), { source: "1", touch_type: "4", function_id: "672", material_id: "67201" });
      }
    ...
    for (const [command, flags, error] of [
      ["ai_translation", {}, /target-language/], ["ai_beauty", {}, /style/], ["color_unite", {}, /reference/],
    ...
    assert.throws(() => prepareTool("ai_beauty", { "retouch-params": '{"skin":1}' }), /retouch-params.*style/);
